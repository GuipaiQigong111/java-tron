...
public class RpcServiceHttp2SecurityTest {
  ...
  public void shouldRejectExcessStreamsBeforeClientAcknowledgesSettings() throws Exception {
    ...
    try (Socket socket = new Socket("127.0.0.1", server.getPort())) {
      ...
      assertRefusedStream(socket.getInputStream(), 5);
    } finally {
      server.shutdownNow();
  ...
  private static void assertRefusedStream(InputStream input, int expectedStreamId)
      throws IOException {
    for (int i = 0; i < 20; i++) {
      Http2Frame frame = readFrame(input);
      if (frame.type == RST_STREAM_FRAME_TYPE && frame.streamId == expectedStreamId) {
    ...
    fail("No REFUSED_STREAM response for stream " + expectedStreamId);
  }
